...
static void assign()
{
	...
	next_token();
	CodeGen(STOREAI, expr(), 0, (ident - 'a')*4); /* ident - 'a' is offset of variable */
}
...
static void print()
{
	...
	if (!is_identifier(token)) {
        ...
        CodeGen(OUTPUTAI, 0, (token-'a')*4, EMPTY_FIELD); /* token - 'a' is offset of varible, *4 for byte address */
	next_token();
}
